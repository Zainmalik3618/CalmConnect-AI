...
const NavButton: React.FC<{
    ...
    >
        {item.icon}
        <span className={`ml-3 overflow-hidden transition-all duration-300 whitespace-nowrap ${isCollapsed ? 'w-0 opacity-0' : 'w-auto opacity-100'}`}>{item.name}</span>
...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setCurrentView, currentUser, onLogout, theme, toggleTheme, isCollapsed, onToggle, totalUnreadCount, unreadAppointmentsCount, unreadNotificationsCount, apiFetch }) => {
  ...
  return (
    <aside className={`relative bg-white dark:bg-gray-800 p-4 flex flex-col border-r border-gray-200 dark:border-gray-700 transition-all duration-300 ease-in-out ${isCollapsed ? 'w-20' : 'w-64'}`}>
       <button 
        onClick={onToggle} 
        className="absolute -right-3 top-9 z-10 p-1 bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-full shadow-md border border-gray-200 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500"
        aria-label={isCollapsed ? "Expand sidebar" : "Collapse sidebar"}
      >
        <ChevronLeftIcon className={`transition-transform duration-300 ${isCollapsed ? 'rotate-180' : 'rotate-0'}`} />
      </button>

      <div className={`flex items-center mb-8 transition-all duration-300 ${isCollapsed ? 'justify-center' : 'justify-start'}`}>
        <LogoIcon />
        <h1 className={`text-xl font-bold ml-2 text-gray-800 dark:text-white overflow-hidden transition-all duration-300 whitespace-nowrap ${isCollapsed ? 'w-0 opacity-0' : 'w-auto opacity-100'}`}>CalmConnect AI</h1>
      </div>
            ...
            {visibleNavItems.map((item, index) => {
                if (item.type === 'divider') {
                    ...
                    if (prevItemExists && nextItemExists) {
                        return <hr key={`divider-${index}`} className={`my-2 border-t border-gray-200 dark:border-gray-700 transition-all duration-300 ${isCollapsed ? 'mx-auto w-8' : 'mx-2'}`} />;
                    }
                    return null;
                }
                return (
      ...
      <div className="mt-auto flex flex-col space-y-1 border-t border-gray-200 dark:border-gray-700 pt-4">
        <button
          ...
          className={`flex w-full items-center p-3 rounded-lg transition-colors text-left ${
            currentView === 'profile'
              ? 'bg-blue-500 text-white'
              : 'text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'
          } ${isCollapsed ? 'justify-center' : ''} focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500`}
        >
            ...
            <span className={`ml-3 text-sm font-medium truncate overflow-hidden transition-all duration-300 whitespace-nowrap ${isCollapsed ? 'w-0 opacity-0' : 'w-auto opacity-100'}`}>{currentUser.username}</span>
        </button>
        <ThemeToggle theme={theme} toggleTheme={toggleTheme} isCollapsed={isCollapsed} />
        <button
          onClick={() => setIsReportModalOpen(true)}
          title={isCollapsed ? "Report Issue" : undefined}
          className={`flex items-center p-3 rounded-lg transition-colors text-left text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 ${isCollapsed ? 'justify-center' : ''} focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500`}
        >
          <AlertCircleIcon />
          <span className={`ml-3 overflow-hidden transition-all duration-300 whitespace-nowrap ${isCollapsed ? 'w-0 opacity-0' : 'w-auto opacity-100'}`}>Report Issue</span>
        </button>
        <button
        ...
        >
          <LogoutIcon />
          <span className={`ml-3 overflow-hidden transition-all duration-300 whitespace-nowrap ${isCollapsed ? 'w-0 opacity-0' : 'w-auto opacity-100'}`}>Logout</span>
        </button>
        <div className={`text-center text-xs text-gray-400 dark:text-gray-500 pt-2 overflow-hidden transition-all duration-300 whitespace-nowrap ${isCollapsed ? 'h-0 opacity-0' : 'h-auto opacity-100'}`}>
          <p>&copy; 2025 CalmConnect AI</p>
        </div>
      </div>
