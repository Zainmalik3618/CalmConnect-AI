...
const MoodTrackerView: React.FC<MoodTrackerViewProps> = ({ currentUser, moodHistory, setMoodHistory, apiFetch, isLoading }) => {
  ...
  if (isLoading) {
    return (
      <div className="p-6 md:p-8 h-full">
        ...
        <div className="surface-motion bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md mb-8">
  ...
  return (
    <div className="p-6 md:p-8 h-full overflow-y-auto">
          ...
          <div className="flex justify-around items-center">
            {moodOptions.map(({ level, emoji, label }) => (
              <button
                ...
                className="flex flex-col items-center group space-y-2 text-gray-600 dark:text-gray-300 hover:text-blue-500 dark:hover:text-blue-400 transition-all duration-200 ease-out transform hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-full p-2"
                aria-label={label}
              >
                <span className="text-4xl md:text-5xl transition-transform duration-300 ease-out group-hover:scale-105" aria-hidden="true">{emoji}</span>
      ...
      {moodHistory.length > 0 && (
        <div className="surface-motion bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md">
